...
export async function proxy(request: NextRequest) {
  ...
  if (
    blockedV1Prefixes.some(
      (blockedPath) =>
        pathWithoutLocale === blockedPath ||
        pathWithoutLocale.startsWith(`${blockedPath}/`)
    )
  ) {
    return new NextResponse('Not found', { status: 404 });
  }

  // Handle internationalization after v1 route blocking.
